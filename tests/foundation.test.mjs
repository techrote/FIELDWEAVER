...
import { APP_VERSION, RENDERER_ISSUE } from '../src/version.js';

test('foundation snapshot reports implemented substrate and renderer as ready', () => {
  ...
  assert.equal(snapshot.phase, RENDERER_ISSUE);
  ...
  assert.equal(snapshot.subsystemStatus.filter((item) => item.state === 'ready').length, 5);
