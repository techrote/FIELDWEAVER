...
FW-014 reran the structural renderer profile in GitHub Actions on **Node 22.23.2, Linux/x64** using 63,726 synthetic retained depositions (95,589 generated vertices, 2,676,492 prepared bytes). One geometry build took **40.440 ms**. Across 120 cached pan/zoom transform rounds at the original 1118×710@1.13 viewport, median/p95 preparation was **0.0016/0.0048 ms**; across a separate 120-round **1920×1080@1x** target viewport it was **0.0016/0.0019 ms**. Exactly **one** artwork geometry rebuild occurred across both workloads. These shared-runner timings are diagnostic rather than a desktop FPS promise, but the structural regression gate directly proves that view-only interaction no longer repeats the hundreds-of-milliseconds geometry build that caused FW-016.

FW-014 also profiles canonical CPU simulation with `npm run benchmark:release`. On the same Node 22.23.2 Linux/x64 runner, the scenario reaches exactly **30,000 long-lived active agents** by tick 470 with 125,872 retained depositions. After one warm-up, three measured runs took **5803.914 / 5900.580 / 5951.430 ms** (min/median/max), for a median **12.5544 ms/tick**. All three samples reproduced state hash `347154762491a6dd` and deposition hash `fc4a907420c6275a`; workload pressure caused 80 explicitly counted dropped spawn attempts after fixed capacity was reached. Timing is noncanonical, while repeated hash equality and the exact 30k workload are assertions.

CI preserves `render-profile.txt` and `release-profile.txt` as the `fw-release-performance-node-22` artifact. No universal frame-rate or milliseconds/tick promise is inferred from shared runners. Representative browser rendering is separately exercised through Chrome and Firefox WebGL2 smoke; headless browser lanes use a 1280×820 test window, while the cached-view profile covers the documented 1920×1080 target geometry-transform workload. Real GPU/display performance remains workload/driver dependent, and correctness is guarded by deterministic hashes plus structural cache assertions rather than fragile CI timing thresholds.
